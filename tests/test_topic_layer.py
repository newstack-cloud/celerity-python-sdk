...
from celerity.resources.topic.layer import TopicLayer
...
class TestTopicLayer:
    ...
    @pytest.mark.asyncio
    @patch("celerity.resources.topic.layer.create_topic_client")
    async def test_registers_single_resource_with_default(
        ...
        assert "celerity:topic:my-topic" in tokens
        assert "celerity:topic:default" in tokens
    ...
    async def test_no_default_for_multiple_resources(
        ...
        tokens = [call.args[0] for call in container.register_value.call_args_list]
        assert "celerity:topic:orders" in tokens
        assert "celerity:topic:notifications" in tokens
        assert "celerity:topic:default" not in tokens
